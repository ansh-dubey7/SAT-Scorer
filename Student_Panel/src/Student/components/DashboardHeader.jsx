...
const Header = ({ onMenuClick }) => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      // Optionally update AuthContext user data
  ...
  const toggleExamsDropdown = (e) => {
    e.preventDefault();
    console.log('Toggling exams dropdown, current state:', isExamsOpen);
  ...
  const toggleProfileDropdown = (e) => {
    e.preventDefault();
    console.log('Toggling profile dropdown, current state:', isProfileOpen);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (examsDropdownRef.current && !examsDropdownRef.current.contains(event.target)) {
        console.log('Clicked outside exams dropdown, closing it');
        setIsExamsOpen(false);
      }
      if (profileDropdownRef.current && !profileDropdownRef.current.contains(event.target)) {
        console.log('Clicked outside profile dropdown, closing it');
        setIsProfileOpen(false);
      }
    };
  ...
  if (loading) {
    return (
      <header className="bg-gradient-to-r from-indigo-900 to-purple-900 text-white sticky top-0 z-50 h-16 font-sans">
  ...
  return (
    <header className="bg-gradient-to-r from-indigo-900 to-purple-900 text-white sticky top-0 z-50 h-16 font-sans shadow-md">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 h-full">
        ...
        <div className="flex items-center space-x-4">
          ...
          {user ? (
            <>
              <Link
                to="/studentdashboard/notifications"
