...
const UpdateCourseContentForm = ({ course }) => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/api/video/course/${course._id}`,
    ...
    const fetchNotes = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/api/notes/course/${course._id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
    ...
    if (editIndex.type === 'video' && videoTitle && videoURL) {
      ...
      try {
        const response = await axios.put(
          `http://localhost:5000/api/video/${editIndex.id}`,
      ...
      try {
        const response = await axios.put(
          `http://localhost:5000/api/notes/${editIndex.id}`,
